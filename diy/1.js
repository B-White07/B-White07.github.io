 ...
 * @LastEditTime: 2024-03-13 10:28:40
...
// ! 加密密钥（长度必须是 16 的整数��，此处�? 32 位）
const secretKey = "httishere9607281";

/**
 * ASE解密
 * @description 使用加密秘钥，对 霢�要解密的参数 进行解密
 * @param {string} encryptedWord - 霢�要解密的参数
 * @param {string} key - 加密密钥（长度必须是 16 的整数��）
...
function aesDecryptParams(CryptoJS, encryptedWord, key = secretKey) {
  ...
  let decrypted = udata.toString(enc.Utf8); // 返回的是加密之前的原始数�?,是字符串类型
  return decrypted;
}
...
function addWatermark() {
  const name = "�? Powered by <a href='https://httishere.gitee.io/widverse/#/lib/time' style='color: #888F47'>Widverse·汪汪汪世�?</a>";
